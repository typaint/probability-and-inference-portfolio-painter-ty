...
goat = rep(NA, R) # both strategies = goat
set.seed(120320)
for (i in seq_along(strategy1)) {
  strategy1[i] = game(4)[1] # assign strategy 1 result
  strategy2[i] = game(4)[2] # assign strategy 2 result
  goat[i] <- ifelse(strategy1[i]=="goat" & strategy2[i]=="goat", 1,0) # determine if both strategies = goat
}

mean(goat) # calculate probability both result in goat
...
for (i in seq_along(median_draws)){
  two_five[i] = quantile(exp(rnorm(N,mean=0,sd=1)), 0.25)
  median_draws[i] = median(exp(rnorm(N,mean=0,sd=1)))
  seven_five[i] = quantile(exp(rnorm(N,mean=0,sd=1)), 0.75)
}

hist(two_five, breaks = 50,
     xlab = "Blood Chemistry Measure",
     main = "Log Normal Distribution, 25th Percentile
     (μ = 0, σ = 1)")
quantile(two_five, probs = c(0.025, 0.975))

hist(median_draws, breaks = 50,
     xlab = "Blood Chemistry Measure",
     main = "Log Normal Distribution, Median
     (μ = 0, σ = 1)")
quantile(median_draws, probs = c(0.025, 0.975))
hist(seven_five, breaks = 50,
     xlab = "Blood Chemistry Measure",
     main = "Log Normal Distribution, 75th Percentile
     (μ = 0, σ = 1)")
quantile(seven_five, probs = c(0.025, 0.975))
```
