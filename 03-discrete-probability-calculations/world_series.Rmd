...
pnbinom(3, 4, .55)
```
I used pnbinom() to calculate the probability of the Braves winning 4 games while losing 3 or fewer games, with a individual head-to-head winning probability of 0.55.
...
```
In the graph above I calculated the probability of the Braves winning the World Series given the Braves have a certain head-to-head winning percentage ranging from 0.5 to 1.
...
series_length
```
For the Braves to achieve a World Series win probability over 0.8 with a head-to-head win probability of 0.55, they would need to play a best-of-71 game series. They would have to win 36 games (wins_needed) and could lose no more than 35 games (wins_needed - 1)
...
```
In the graph above, I found the shortest series of games that the Yankees and Braves would have to play, based on the Braves head-to-head win probability, to ensure the Braves probability of winning the World Series is 0.8.
